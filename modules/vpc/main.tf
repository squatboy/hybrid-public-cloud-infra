...
#------------------------------------------------------------------------------
# NAT Gateway (Conditional - for Test Environment)
#------------------------------------------------------------------------------
...
# Private Route Table
# NAT Gateway가 활성화되면 NAT로, 비활성화되면 IGW로 라우팅
resource "aws_route_table" "private" {
  vpc_id = aws_vpc.this.id

  route {
    cidr_block     = "0.0.0.0/0"
    gateway_id     = var.enable_nat_gateway ? null : aws_internet_gateway.this.id
